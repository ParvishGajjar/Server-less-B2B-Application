const admin = require('firebase-admin');
const functions = require('firebase-functions')
const cors = require('cors')({ origin: true });
const { Storage } = require('@google-cloud/storage');
const os = require('os');
const createCsvWriter = require('csv-writer').createObjectCsvWriter;
const storage = new Storage();

admin.initializeApp({
  credential: admin.credential.cert({
    "type": "service_account",
    "project_id": "csci5410-assignment4-fenil",
    "private_key_id": "5dc271e745d9726ab3fbcb8f09fbab3d9146f427",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n6PUqJU71QjxBQ2tzvy4tsP5k\n-----END PRIVATE KEY-----\n",
    "client_email": "firebase-adminsdk-qbdbf@csci5410-assignment4-fenil.iam.gserviceaccount.com",
    "client_id": "101054428939321940300",
    "auth_uri": "https://accounts.google.com/o/oauth2/auth",
    "token_uri": "https://oauth2.googleapis.com/token",
    "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
    "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-qbdbf%40csci5410-assignment4-fenil.iam.gserviceaccount.com"
  })
});


exports.helloWorld = functions.https.onRequest(async (req, res) => {
  await cors(req, res, async () => {
    // your function body here - use the provided req and res from cors
    const fetchdata = await admin.firestore().collection('userTours').get();
    let result = []
    fetchdata.forEach(doc => {
      result.push({ tourId: doc.data().tourId, userId: doc.data().userId, tourCost: doc.data().tourCost, tourDays: doc.data().tourDays, tourGuests: doc.data().tourGuests })
    })
    const tmpDir = os.tmpdir();
    const csvWriter = createCsvWriter({
      path: tmpDir + '/dataVector.csv',
      header: [
        { id: 'tourId', title: 'tourId' },
        { id: 'userId', title: 'userId' },
        { id: 'tourCost', title: 'tourCost' },
        { id: 'tourDays', title: 'tourDays' },
        { id: 'tourGuests', title: 'tourGuests' },
      ]
    });
    csvWriter
      .writeRecords(result)
      .then(async () => {
        console.log("File created")
        await storage.bucket("usertours-serverless-2").upload(tmpDir + '/dataVector.csv', {
          destination: "dataVector.csv",
        });
        console.log("File uploaded successfully");
      });
    res.status(200).json({ data: true, message: 'successs' })
  })
});
